...
namespace ClaimAPI.Services
{
...

    {
        ...
        Tuple<string, List<UserName>> GetUser();
    }
    public class UserService : ICRUD<UserVm>,IEntity
    {
        ...
        public Tuple<string, List<UserName>> GetUser()
        {
            string message = string.Empty;
            ...
            try
            {
                using (var command = DbContext.Database.GetDbConnection().CreateCommand())
                {
                    ...
                    while (result.Read())
                    {
                        UserName objUser = new UserName();
                        objUser.Name = result["Nm"].ToString();
                        objUser.Id = Convert.ToInt32(result["Id"]);
                        objUser.Role = result["Role"].ToString();
            ...
            return new Tuple<string, List<UserName>>(message, lstUser);
        }
    }
}
